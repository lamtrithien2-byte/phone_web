...
public class AuthController {
    ...
    public String login(
        ...
        try {
            response = authApiClient.login(new LoginRequest(username, password, role));
        } catch (RestClientException ex) {
            model.addAttribute("error", "Core API is unavailable");
            return "login";
        }
        if (response == null || !response.isSuccess() || response.data() == null) {
            model.addAttribute("error", response != null ? response.message() : "Core API is unavailable");
            return "login";
        }
        ...
        if ("admin".equalsIgnoreCase(response.data().roleName())) {
            return "redirect:/dashboard";
        }
        return "redirect:/sales";
    }
    ...
    public String resetPassword(@RequestParam String token,
        ...
        try {
            ...
            model.addAttribute("success", "Dat lai mat khau thanh cong");
            return "reset-password";
        } catch (RestClientException ex) {
            model.addAttribute("error", "Core API is unavailable");
            model.addAttribute("token", token);
