...
<#
  ...
  Reset and re-import DB explicitly:
    .\start-phone-store.ps1 -ReinitializeDatabase
#>
param(
    [string] $MysqlPassword = "",
    [switch] $ReinitializeDatabase,
    [switch] $WithCms
)
...
if (-not (Test-PortOpen -Port 3306)) {
    Write-Host @"
Loi: MySQL chua lang nghe tren localhost:3306.
...
$mysql = Find-MysqlExe
$skipDatabaseInit = -not $ReinitializeDatabase
if (-not $skipDatabaseInit -and $mysql) {
    $sqlFile = Join-Path $ScriptDir "phone_store_full_setup.sql"
    Write-Warning "Ban dang chon reset lai database phone_store. Du lieu hien co se bi xoa."
...
if ($skipDatabaseInit) {
    Write-Host "Bo qua import SQL de giu nguyen database hien co. Dung -ReinitializeDatabase neu muon tao lai DB." -ForegroundColor DarkGray
}
...
Write-Host "  Shop (khach):   http://localhost:8083/"
Write-Host "  API:            http://localhost:8081/api/products"
if ($WithCms) {
    Write-Host "  CMS (quan tri): http://localhost:8082/login  (admin / admin123 - lan dau dang nhap vai tro Admin)"
}
Write-Host ""
